...
import (
	...
	ordersClient "github.com/transcom/milmove_orders/pkg/gen/ordersclient"
...
func CreateClient(v *viper.Viper) (*ordersClient.MilmoveOrders, *pksigner.Store, error) {
	...
	myRuntime := runtimeClient.NewWithClient(hostWithPort, ordersClient.DefaultBasePath, []string{"https"}, httpClient)
	myRuntime.EnableConnectionReuse()
	myRuntime.SetDebug(verbose)

	ordersGateway := ordersClient.New(myRuntime, nil)

	return ordersGateway, store, nil
}
